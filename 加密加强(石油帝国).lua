--1
local Script = [[
loadstring(game:HttpGet("https://raw.githubusercontent.com/HB-ksdb/CNM-LKJ/main/石油帝国加密.txt"))()
]]

local encoded = Script:gsub(".", function(bb) return "\\" .. bb:byte() end) or Script .. "\""
print('-- (loadstring)')
print('loadstring("'..encoded..'")()')
--2
local _local = {} --存储ASCII值的表
local _string = [[loadstring(game:HttpGet("https://raw.githubusercontent.com/HB-ksdb/CNM-LKJ/main/石油帝国加密.txt"))())]] --脚本放这里

for _z = 1, #_string do --转换为其ASCII值并将其存储在表中
    _local[_z] = string.byte(_string, _z)
end

local _x = "" --用于保存转换后的十六进制字符串的变量
for _y = 1, #_local do --将ASCII值转换为十六进制字符串并将其连接
    _x = _x .. "\\x" .. string.format("%X", _local[_y]) --%X：将ASCII值转换为大写十六进制格式
end
-- 打印转换后的十六进制字符串
print('"'.._x..'"')
--3
